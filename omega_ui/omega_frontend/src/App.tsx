...
interface FootprintData {
  ...
  deep_trades?: DeepTrade[];
  delta_divergences?: any[];
  real_fvgs?: RealFVG[];
  cvd_values?: any[];
  heatmap?: any;
  volume_profile?: any;
...
const MMTFootprintChart = ({ data, symbol }: { data: FootprintData | null, symbol: string }) => {
  ...
  // Get signals for visible candles
  const visibleAbsorptions = (data.absorptions || []).filter(a => a.bar_index >= startIdx);
  ...
  const bullishSignals = visibleAbsorptions.filter(a => a.type === 'BULLISH').length + 
                         visibleDivergences.filter(d => d.type === 'BULLISH').length +
                         visibleDeepTrades.filter(t => t.type === 'BUY').length;
  const bearishSignals = visibleAbsorptions.filter(a => a.type === 'BEARISH').length + 
          ...
          <div className="flex-1 flex overflow-x-auto">
            ...
            <div className="flex-1 flex gap-px p-1 overflow-x-auto relative">
              ...
              {realFVGs.slice(0, 3).map((fvg, idx) => {
                ...
                const height = ((fvg.price_high - fvg.price_low) / priceRange) * 100;
              ...
              {displayCandles.map((candle, candleIdx) => {
                ...
                const hasAbsorption = visibleAbsorptions.some(a => a.bar_index === globalIdx);
                const hasDivergence = visibleDivergences.some(d => d.bar_index === globalIdx);
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
            ...
            <div className="bg-slate-800/50 rounded-lg p-2 border border-yellow-500/20">
              ...
              {visibleAbsorptions.map((a, idx) => (
                <div key={idx} className={`text-[9px] p-1.5 rounded mb-1 ${
                  ...
                  <div className="text-slate-500">Strength: {a.strength}%</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="bg-slate-800/50 rounded-lg p-2 border border-cyan-500/20">
              ...
              {visibleDeepTrades.map((t, idx) => (
                <div key={idx} className={`text-[9px] p-1.5 rounded mb-1 ${
                  ...
                  <div className="text-slate-500">Volume: {formatVol(t.volume)} ({t.multiplier}x avg)</div>
                </div>
              ))}
            </div>
          )}
          ...
          {visibleAbsorptions.length === 0 && visibleDivergences.length === 0 && visibleDeepTrades.length === 0 && realFVGs.length === 0 && (
            <div className="text-center text-slate-500 py-8">
              ...
              <div className="text-xs">Waiting for institutional activity...</div>
            </div>
          )}
        </div>
      )}
...
function App() {
  ...
  const _getRegimeColor = (regime: string) => {
  ...
  void _getRegimeColor;

  const _getStructureIcon = (structure: string) => {
  ...
  return (
        <div className="min-h-screen bg-black text-white overflow-hidden">
          {/* MINIMAL HEADER - No frames */}
          <header className="px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <Brain className="w-6 h-6 text-cyan-400" />
            <span className="text-lg font-bold text-white">OMEGA-DEVIN</span>
          </div>
          
          {/* Symbol selector - minimal */}
          <div className="flex items-center gap-1">
            {symbols.map(symbol => (
              <button
                key={symbol}
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            <div className={`flex items-center gap-1 ${state?.kill_switch_active ? 'text-red-400' : 'text-emerald-400'}`}>
              <Shield className="w-3 h-3" />
              {state?.kill_switch_active ? 'HALTED' : 'ACTIVE'}
            </div>
            {/* Confidence */}
            <div className="text-slate-400">
              conf <span className="text-white font-mono">{decision?.confidence?.toFixed(0) || 0}%</span>
            </div>
          </div>
        </div>
      </header>

      {/* MAIN LAYOUT - Chart dominates, no frames */}
      <div className="flex" style={{ height: 'calc(100vh - 52px)' }}>
        
        {/* LEFT SIDEBAR - Minimal, no frames */}
        <div className="w-32 p-2 flex flex-col gap-4 text-xs">
          {/* Account */}
          <div>
            <div className="text-slate-500 mb-1">CAPITAL</div>
            <div className="text-lg font-mono font-bold text-white">{formatCurrency(state?.capital || 10000)}</div>
          </div>
          <div>
            ...
            <div className={`text-xs ${(state?.total_pnl_pct || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
              {formatPercent(state?.total_pnl_pct || 0)}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="text-white font-mono">{performance?.total_trades || 0}</div>
          </div>
          ...
          <div>
            ...
            <div className={`font-bold ${
              ...
              {chartData?.market_structure?.structure || 'NEUTRAL'}
            </div>
          </div>
          <div>
            <div className="text-slate-500 mb-1">REGIME</div>
            <div className={`font-medium ${
              state?.regime === 'TRENDING' ? 'text-emerald-400' :
              state?.regime === 'VOLATILE' ? 'text-red-400' : 'text-amber-400'
            }`}>
        ...
        {/* CENTER - CHART DOMINATES */}
        <div className="flex-1 flex flex-col p-2">
          {/* Chart header - minimal */}
          <div className="flex items-center justify-between mb-1 px-2">
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span>{selectedSymbol} Price Chart with VWAP</span>
              {decision && !decision.can_trade && (
                <span className="text-amber-400">NO TRADE - {decision.provenance_check}</span>
              )}
            </div>
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span className="flex items-center gap-1"><span className="w-3 h-0.5 bg-purple-500"></span> VWAP</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 bg-emerald-500"></span> Bullish</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 bg-red-500"></span> Bearish</span>
            </div>
          </div>
          
          {/* MAIN CHART - Takes 75% of height */}
          <div className="flex-1 relative" style={{ minHeight: '70%' }}>
            <NoTradeFog
              decision={decision}
            ...
            <CandlestickChart 
              ...
              unifiedDecision={(() => {
                ...
                let decisionText = 'WAIT';
                if (score > 0.5) decisionText = 'STRONG BUY';
                else if (score > 0.2) decisionText = 'BUY';
                else if (score < -0.5) decisionText = 'STRONG SELL';
                else if (score < -0.2) decisionText = 'SELL';
                
                return { decision: decisionText, confidence: Math.abs(score) * 100, signals };
              })()}
              tradeSetup={(() => {
                const vp = chartData?.volume_profile;
                const of = chartData?.order_flow_imbalance;
          ...
          {/* BOTTOM ROW - Delta charts side by side, minimal */}
          <div className="flex gap-2 mt-2" style={{ height: '25%' }}>
            <div className="flex-1">
              <div className="text-xs text-slate-500 mb-1 px-1">Bar Delta</div>
              <DeltaChart data={chartData?.delta || []} />
            </div>
            <div className="flex-1">
              <div className="text-xs text-slate-500 mb-1 px-1">Cumulative Delta</div>
              <CumulativeDeltaChart data={chartData?.delta || []} />
            </div>
          </div>
        </div>

        {/* RIGHT SIDEBAR - Intelligence, no frames */}
